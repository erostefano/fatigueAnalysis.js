const { loadImagesFromFolder, splitIntoTrainingData, getDataSet } = require("./util");
...
const xTrain = tf.stack(eyesOpenTraining.images.concat(eyesClosedTraining.images));
logger.info('xTrain shape', xTrain.shape);

const yTrain = tf.tensor2d(eyesOpenTraining.labels.concat(eyesClosedTraining.labels), [xTrain.shape[0], 3]);
